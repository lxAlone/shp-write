...
module.exports = function (gj, options, stream = false) {
	...
	[([geojson.point(gj)], [geojson.line(gj), geojson.multiline(gj)], [geojson.polygon(gj), geojson.multipolygon(gj)])].forEach(function (l) {
		if ((l[0].geometries.length && l[0].geometries[0].length) || (l[1] && l[1].geometries.length && l[1].geometries[0].length)) {
			...
			write(
				...
				function (err, files) {
					...
					} else if (options && options.types && options.types[type.toLowerCase()]) {
						fileName = options.types[type.toLowerCase()];
					} else {
						fileName = type;
					}
